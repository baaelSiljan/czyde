...
this.defaultMaterial=new JSC3D.Material;this.defaultMaterial.ambientColor=0;this.defaultMaterial.diffuseColor=this.modelColor;this.defaultMaterial.transparency=0;this.defaultMaterial.simulateSpecular=true;this.drawBackground();this.loadScene()};a.update=function(b){if(this.isFailed)this.reportError(this.errorMsg);else if(this.scene){if(!b&&this.colorBuffer!=null){this.beginScene();this.render();this.endScene()}this.paint()}else this.drawBackground()};
a.rotate=function(b,c,f){this.rotMatrix.rotateAboutXAxis(b);this.rotMatrix.rotateAboutYAxis(c);this.rotMatrix.rotateAboutZAxis(f)};a.setRenderMode=function(b){this.renderMode=this.params.RenderMode=b};
...
a.pick=function(b,c){var f=new JSC3D.PickInfo,g=this.canvas.getBoundingClientRect();b=b-g.left;c=c-g.top;if(this.selectionBuffer!=null&&b>=0&&b<this.canvas.width&&c>=0&&c<this.canvas.height){switch(this.definition){case "low":b=~~(b/2);c=~~(c/2);break;case "high":b*=2;c*=2;break;case "standard":default:break}g=this.selectionBuffer[c*this.frameWidth+b];if(g>0)for(var d=this.scene.getChildren(),e=0;e<d.length;e++)if(d[e].internalId==g){f.mesh=d[e];break}}f.canvasX=b;f.canvasY=c;if(f.mesh)f.depth=this.zBuffer[c*
...
a.generateBackground=function(){for(var b=this.frameWidth,c=this.frameHeight,f=this.bkgColorBuffer,g=(this.bkgColor1&16711680)>>16,d=(this.bkgColor1&65280)>>8,e=this.bkgColor1&255,h=(this.bkgColor2&16711680)>>16,i=(this.bkgColor2&65280)>>8,m=this.bkgColor2&255,n=0,q=0;q<c;q++)for(var j=g+q*(h-g)/c&255,y=d+q*(i-d)/c&255,k=e+q*(m-e)/c&255,R=0;R<b;R++)f[n++]=j<<16|y<<8|k};a.drawBackground=function(){if(this.canvasData){this.beginScene();this.endScene();this.paint()}};
...
a.endScene=function(){var b=this.canvasData.data,c=this.canvas.width,f=this.canvas.height,g=this.colorBuffer,d=this.frameWidth,e=d*this.frameHeight;switch(this.definition){case "low":var h=c>>1,i=d-h,m=0,n=0;for(e=0;e<f;e++){for(var q=0;q<c;q++){d=g[m];b[n]=(d&16711680)>>16;b[n+1]=(d&65280)>>8;b[n+2]=d&255;b[n+3]=255;m+=q&1;n+=4}m+=e&1?i:-h}break;case "high":for(e=n=m=0;e<f;e++){for(q=0;q<c;q++){h=g[m];i=g[m+1];var j=g[m+d],y=g[m+d+1];b[n]=(h&16711680)+(i&16711680)+(j&16711680)+(y&16711680)>>18;b[n+
1]=(h&65280)+(i&65280)+(j&65280)+(y&65280)>>10;b[n+2]=(h&255)+(i&255)+(j&255)+(y&255)>>2;b[n+3]=255;m+=2;n+=4}m+=d}break;case "standard":default:for(n=m=0;m<e;m++,n+=4){d=g[m];b[n]=(d&16711680)>>16;b[n+1]=(d&65280)>>8;b[n+2]=d&255;b[n+3]=255}break}};
...
f.vertexBuffer,f.transformedVertexBuffer);if(f.visible)switch(this.renderMode){case "point":this.renderPoint(f);break;case "flat":this.renderSolidFlat(f);break;case "smooth":this.renderSolidSmooth(f);break;case "texture":f.hasTexture()?this.renderSolidTexture(f):this.renderSolidFlat(f);break;case "textureflat":f.hasTexture()?this.renderTextureFlat(f):this.renderSolidFlat(f);break;case "texturesmooth":f.hasTexture()?this.renderTextureSmooth(f):this.renderSolidSmooth(f);break;default:this.renderSolidFlat(f);
break}}}}};
a.sortScene=function(b){for(var c=[],f=this.scene.getChildren(),g=0;g<f.length;g++){var d=f[g];if(!d.isTrivial()){c.push(d);var e=d.aabb.center();JSC3D.Math3D.transformVectors(b,e,e);d.sortKey={depth:e[2],isTransparnt:(d.material?d.material:this.defaultMaterial).transparency>0||(d.hasTexture()?d.texture.hasTransparency:false)}}}c.sort(function(h,i){if(!h.sortKey.isTransparnt&&i.sortKey.isTransparnt)return-1;if(h.sortKey.isTransparnt&&!i.sortKey.isTransparnt)return 1;if(h.sortKey.isTransparnt)return h.sortKey.depth-i.sortKey.depth;
return i.sortKey.depth-h.sortKey.depth});return c};
a.renderPoint=function(b){var c=this.frameWidth,f=c-1,g=this.frameHeight-1,d=b.transformedVertexBuffer,e=b.transformedVertexNormalZBuffer,h=this.colorBuffer,i=this.zBuffer,m=this.selectionBuffer,n=d.length/3,q=b.internalId,j=b.material?b.material.getPalette():this.defaultMaterial.getPalette();if(!e||e.length<n){b.transformedVertexNormalZBuffer=new Array(n);e=b.transformedVertexNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.vertexNormalBuffer,e);for(var y=0,k=0;y<n;y++,k+=3){var R=e[y];
if(b.isDoubleSided)R=R>0?R:-R;if(R>0){var C=~~(d[k]+0.5),s=~~(d[k+1]+0.5),E=d[k+2];if(C>=0&&C<f&&s>=0&&s<g){C=s*c+C;R=j[~~(R*255)];if(E>i[C]){i[C]=E;h[C]=R;m[C]=q}C++;if(E>i[C]){i[C]=E;h[C]=R;m[C]=q}C+=f;if(E>i[C]){i[C]=E;h[C]=R;m[C]=q}C++;if(E>i[C]){i[C]=E;h[C]=R;m[C]=q}}}}};
a.renderSolidFlat=function(b){var c=this.frameWidth,f=this.frameHeight,g=b.indexBuffer,d=b.transformedVertexBuffer,e=b.transformedFaceNormalZBuffer,h=this.colorBuffer,i=this.zBuffer,m=this.selectionBuffer,n=b.faceCount,q=b.internalId,j=b.material?b.material:this.defaultMaterial,y=j.getPalette(),k=j.transparency==0,R=j.transparency*255,C=255-R;if(j.transparency!=1){if(!e||e.length<n){b.transformedFaceNormalZBuffer=new Array(n);e=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,
b.faceNormalBuffer,e);j=new Array(3);for(var s=new Array(3),E=new Array(3),G=0,x=0;G<n;){var t=e[G++];if(b.isDoubleSided)t=t>0?t:-t;if(t<0){do;while(g[x++]!=-1)}else{t=y[~~(t*255)];var u,v,o;u=g[x++]*3;v=g[x++]*3;do{o=g[x++]*3;j[0]=~~(d[u]+0.5);s[0]=~~(d[u+1]+0.5);E[0]=d[u+2];j[1]=~~(d[v]+0.5);s[1]=~~(d[v+1]+0.5);E[1]=d[v+2];j[2]=~~(d[o]+0.5);s[2]=~~(d[o+1]+0.5);E[2]=d[o+2];v=s[0]<s[1]?0:1;v=s[v]<s[2]?v:2;var r=s[0]>s[1]?0:1;r=s[r]>s[2]?r:2;var D=3-r-v;if(v!=r){var p=j[r],I=E[r],l=s[r]-s[v];l=l!=
0?l:1;var U=(j[r]-j[v])/l;l=(E[r]-E[v])/l;var N=j[r],w=E[r],z=s[r]-s[D];z=z!=0?z:1;var F=(j[r]-j[D])/z;z=(E[r]-E[D])/z;var K=j[D],H=E[D],B=s[D]-s[v];B=B!=0?B:1;var sa=(j[D]-j[v])/B;B=(E[D]-E[v])/B;var M=s[r]*c;for(r=s[r];r>s[v];r--){if(r>=0&&r<f){var S=~~p,A=I,V,ka;if(r>s[D]){V=~~N;ka=w}else{V=~~K;ka=H}if(S>V){var aa;aa=S;S=V;V=aa;aa=A;A=ka;ka=aa}if(S<0)S=0;if(V>=c)V=c-1;ka=S!=V?(ka-A)/(V-S):1;aa=M+S;if(k){S=S;for(A=A;S<=V;S++,A+=ka){if(A>i[aa]){i[aa]=A;h[aa]=t;m[aa]=q}aa++}}else{S=S;for(A=A;S<V;S++,
A+=ka){if(A>i[aa]){var ha=h[aa];h[aa]=(ha&16711680)*R+(t&16711680)*C>>8&16711680|(ha&65280)*R+(t&65280)*C>>8&65280|(ha&255)*R+(t&255)*C>>8&255;m[aa]=q}aa++}}}p-=U;I-=l;if(r>s[D]){N-=F;w-=z}else{K-=sa;H-=B}M-=c}}v=o}while(g[x]!=-1);x++}}}};
a.renderSolidSmooth=function(b){var c=this.frameWidth,f=this.frameHeight,g=b.indexBuffer,d=b.transformedVertexBuffer,e=b.transformedVertexNormalZBuffer,h=b.transformedFaceNormalZBuffer,i=this.colorBuffer,m=this.zBuffer,n=this.selectionBuffer,q=b.faceCount,j=d.length/3,y=b.internalId,k=b.material?b.material:this.defaultMaterial,R=k.getPalette(),C=k.transparency==0,s=k.transparency*255,E=255-s;if(k.transparency!=1){if(!e||e.length<j){b.transformedVertexNormalZBuffer=new Array(j);e=b.transformedVertexNormalZBuffer}if(!h||
h.length<q){b.transformedFaceNormalZBuffer=new Array(q);h=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.vertexNormalBuffer,e);JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.faceNormalBuffer,h);b=b.isDoubleSided;j=new Array(3);k=new Array(3);for(var G=new Array(3),x=new Array(3),t=0,u=0;t<q;){var v=h[t++];if(b)v=v>0?v:-v;if(v<0){do;while(g[u++]!=-1)}else{var o,r,D,p,I;v=g[u++];D=v*3;o=g[u++];p=o*3;do{r=g[u++];I=r*3;j[0]=~~(d[D]+0.5);k[0]=~~(d[D+1]+0.5);G[0]=d[D+2];
j[1]=~~(d[p]+0.5);k[1]=~~(d[p+1]+0.5);G[1]=d[p+2];j[2]=~~(d[I]+0.5);k[2]=~~(d[I+1]+0.5);G[2]=d[I+2];x[0]=e[v];x[1]=e[o];x[2]=e[r];if(b){if(x[0]<0)x[0]=-x[0];if(x[1]<0)x[1]=-x[1];if(x[2]<0)x[2]=-x[2]}o=k[0]<k[1]?0:1;o=k[o]<k[2]?o:2;var l=k[0]>k[1]?0:1;l=k[l]>k[2]?l:2;p=3-l-o;if(o!=l){var U=j[l],N=G[l],w=x[l]*255,z=k[l]-k[o];z=z!=0?z:1;var F=(j[l]-j[o])/z,K=(G[l]-G[o])/z;z=(x[l]-x[o])*255/z;var H=j[l],B=G[l],sa=x[l]*255,M=k[l]-k[p];M=M!=0?M:1;var S=(j[l]-j[p])/M,A=(G[l]-G[p])/M;M=(x[l]-x[p])*255/M;
var V=j[p],ka=G[p],aa=x[p]*255,ha=k[p]-k[o];ha=ha!=0?ha:1;var ma=(j[p]-j[o])/ha,ta=(G[p]-G[o])/ha;ha=(x[p]-x[o])*255/ha;var za=k[l]*c;for(l=k[l];l>k[o];l--){if(l>=0&&l<f){var X=~~U,Y=N,da=w,fa,ra,na;if(l>k[p]){fa=~~H;ra=B;na=sa}else{fa=~~V;ra=ka;na=aa}if(X>fa){var O;O=X;X=fa;fa=O;O=Y;Y=ra;ra=O;O=da;da=na;na=O}ra=X!=fa?(ra-Y)/(fa-X):1;na=X!=fa?(na-da)/(fa-X):1;if(X<0){Y-=X*ra;da-=X*na;X=0}if(fa>=c)fa=c-1;O=za+X;if(C){X=X;Y=Y;for(da=da;X<=fa;X++,Y+=ra,da+=na){if(Y>m[O]){m[O]=Y;i[O]=R[da>0?~~da:0];n[O]=
y}O++}}else{X=X;Y=Y;for(da=da;X<fa;X++,Y+=ra,da+=na){if(Y>m[O]){var la=R[da>0?~~da:0],ya=i[O];i[O]=(ya&16711680)*s+(la&16711680)*E>>8&16711680|(ya&65280)*s+(la&65280)*E>>8&65280|(ya&255)*s+(la&255)*E>>8&255;n[O]=y}O++}}}U-=F;N-=K;w-=z;if(l>k[p]){H-=S;B-=A;sa-=M}else{V-=ma;ka-=ta;aa-=ha}za-=c}}p=I;o=r}while(g[u]!=-1);u++}}}};
a.renderSolidTexture=function(b){var c=this.frameWidth,f=this.frameHeight,g=b.indexBuffer,d=b.transformedVertexBuffer,e=b.transformedFaceNormalZBuffer,h=this.colorBuffer,i=this.zBuffer,m=this.selectionBuffer,n=b.faceCount,q=b.internalId,j=b.texture,y=!j.hasTransparency,k=b.texCoordBuffer,R=b.texCoordIndexBuffer,C=j.data,s=j.width,E=s-1,G=j.hasMipmap()?j.mipmaps:null,x=G?j.mipentries:null;if(!e||e.length<n){b.transformedFaceNormalZBuffer=new Array(n);e=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,
b.faceNormalBuffer,e);for(var t=new Array(3),u=new Array(3),v=new Array(3),o=new Array(3),r=new Array(3),D=0,p=0;D<n;){var I=e[D++];if(b.isDoubleSided)I=I>0?I:-I;if(I<0){do;while(g[p++]!=-1)}else{var l,U,N,w,z;I=g[p]*3;N=R[p]*2;p++;l=g[p]*3;w=R[p]*2;p++;if(G){U=g[p]*3;z=R[p]*2;s=j.width;t[0]=d[I];u[0]=d[I+1];t[1]=d[l];u[1]=d[l+1];t[2]=d[U];u[2]=d[U+1];o[0]=k[N]*s;r[0]=k[N+1]*s;o[1]=k[w]*s;r[1]=k[w+1]*s;o[2]=k[z]*s;r[2]=k[z+1]*s;C=(t[1]-t[0])*(u[2]-u[0])-(u[1]-u[0])*(t[2]-t[0]);if(C<0)C=-C;C+=1;E=
(o[1]-o[0])*(r[2]-r[0])-(r[1]-r[0])*(o[2]-o[0]);if(E<0)E=-E;C=E/C;E=0;if(C<x[1])E=0;else if(C>=x[x.length-1]){E=x.length-1;s=1}else for(;C>=x[E+1];){E++;s/=2}C=G[E];E=s-1}do{U=g[p]*3;z=R[p]*2;p++;t[0]=~~(d[I]+0.5);u[0]=~~(d[I+1]+0.5);v[0]=d[I+2];t[1]=~~(d[l]+0.5);u[1]=~~(d[l+1]+0.5);v[1]=d[l+2];t[2]=~~(d[U]+0.5);u[2]=~~(d[U+1]+0.5);v[2]=d[U+2];o[0]=k[N]*s;r[0]=k[N+1]*s;o[1]=k[w]*s;r[1]=k[w+1]*s;o[2]=k[z]*s;r[2]=k[z+1]*s;l=u[0]<u[1]?0:1;l=u[l]<u[2]?l:2;var F=u[0]>u[1]?0:1;F=u[F]>u[2]?F:2;w=3-F-l;if(l!=
F){var K=t[F],H=v[F],B=o[F],sa=r[F],M=u[F]-u[l];M=M!=0?M:1;var S=(t[F]-t[l])/M,A=(v[F]-v[l])/M,V=(o[F]-o[l])/M;M=(r[F]-r[l])/M;var ka=t[F],aa=v[F],ha=o[F],ma=r[F],ta=u[F]-u[w];ta=ta!=0?ta:1;var za=(t[F]-t[w])/ta,X=(v[F]-v[w])/ta,Y=(o[F]-o[w])/ta;ta=(r[F]-r[w])/ta;var da=t[w],fa=v[w],ra=o[w],na=r[w],O=u[w]-u[l];O=O!=0?O:1;var la=(t[w]-t[l])/O,ya=(v[w]-v[l])/O,wa=(o[w]-o[l])/O;O=(r[w]-r[l])/O;var Ga=u[F]*c;for(F=u[F];F>u[l];F--){if(F>=0&&F<f){var Z=~~K,oa=H,pa=B,J=sa,P,ba,Q,$;if(F>u[w]){P=~~ka;ba=aa;
Q=ha;$=ma}else{P=~~da;ba=fa;Q=ra;$=na}if(Z>P){var T;T=Z;Z=P;P=T;T=oa;oa=ba;ba=T;T=pa;pa=Q;Q=T;T=J;J=$;$=T}ba=Z!=P?(ba-oa)/(P-Z):1;Q=Z!=P?(Q-pa)/(P-Z):1;$=Z!=P?($-J)/(P-Z):1;if(Z<0){oa-=Z*ba;pa-=Z*Q;J-=Z*$;Z=0}if(P>=c)P=c-1;T=Ga+Z;if(y){Z=Z;oa=oa;pa=pa;for(J=J;Z<=P;Z++,oa+=ba,pa+=Q,J+=$){if(oa>i[T]){i[T]=oa;h[T]=C[(J&E)*s+(pa&E)];m[T]=q}T++}}else{Z=Z;oa=oa;pa=pa;for(J=J;Z<P;Z++,oa+=ba,pa+=Q,J+=$){if(oa>i[T]){var ua=C[(J&E)*s+(pa&E)],va=h[T],W=ua>>24&255,L=255-W;h[T]=(va&16711680)*L+(ua&16711680)*W>>
8&16711680|(va&65280)*L+(ua&65280)*W>>8&65280|(va&255)*L+(ua&255)*W>>8&255;m[T]=q}T++}}}K-=S;H-=A;B-=V;sa-=M;if(F>u[w]){ka-=za;aa-=X;ha-=Y;ma-=ta}else{da-=la;fa-=ya;ra-=wa;na-=O}Ga-=c}}l=U;w=z}while(g[p]!=-1);p++}}};
a.renderTextureFlat=function(b){var c=this.frameWidth,f=this.frameHeight,g=b.indexBuffer,d=b.transformedVertexBuffer,e=b.transformedFaceNormalZBuffer,h=this.colorBuffer,i=this.zBuffer,m=this.selectionBuffer,n=b.faceCount,q=b.internalId,j=b.material?b.material:this.defaultMaterial,y=j.getPalette(),k=b.texture,R=j.transparency==0&&!k.hasTransparency,C=~~((1-j.transparency)*255),s=b.texCoordBuffer,E=b.texCoordIndexBuffer,G=k.data,x=k.width,t=x-1,u=k.hasMipmap()?k.mipmaps:null,v=u?k.mipentries:null;if(j.transparency!=
1){if(!e||e.length<n){b.transformedFaceNormalZBuffer=new Array(n);e=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.faceNormalBuffer,e);j=new Array(3);for(var o=new Array(3),r=new Array(3),D=new Array(3),p=new Array(3),I=0,l=0;I<n;){var U=e[I++];if(b.isDoubleSided)U=U>0?U:-U;if(U<0){do;while(g[l++]!=-1)}else{U=y[~~(U*255)];var N,w,z,F,K,H;N=g[l]*3;F=E[l]*2;l++;w=g[l]*3;K=E[l]*2;l++;if(u){z=g[l]*3;H=E[l]*2;x=k.width;j[0]=d[N];o[0]=d[N+1];j[1]=d[w];o[1]=d[w+1];j[2]=d[z];
o[2]=d[z+1];D[0]=s[F]*x;p[0]=s[F+1]*x;D[1]=s[K]*x;p[1]=s[K+1]*x;D[2]=s[H]*x;p[2]=s[H+1]*x;G=(j[1]-j[0])*(o[2]-o[0])-(o[1]-o[0])*(j[2]-j[0]);if(G<0)G=-G;G+=1;t=(D[1]-D[0])*(p[2]-p[0])-(p[1]-p[0])*(D[2]-D[0]);if(t<0)t=-t;G=t/G;t=0;if(G<v[1])t=0;else if(G>=v[v.length-1]){t=v.length-1;x=1}else for(;G>=v[t+1];){t++;x/=2}G=u[t];t=x-1}do{z=g[l]*3;H=E[l]*2;l++;j[0]=~~(d[N]+0.5);o[0]=~~(d[N+1]+0.5);r[0]=d[N+2];j[1]=~~(d[w]+0.5);o[1]=~~(d[w+1]+0.5);r[1]=d[w+2];j[2]=~~(d[z]+0.5);o[2]=~~(d[z+1]+0.5);r[2]=d[z+
2];D[0]=s[F]*x;p[0]=s[F+1]*x;D[1]=s[K]*x;p[1]=s[K+1]*x;D[2]=s[H]*x;p[2]=s[H+1]*x;w=o[0]<o[1]?0:1;w=o[w]<o[2]?w:2;var B=o[0]>o[1]?0:1;B=o[B]>o[2]?B:2;K=3-B-w;if(w!=B){var sa=j[B],M=r[B],S=D[B],A=p[B],V=o[B]-o[w];V=V!=0?V:1;var ka=(j[B]-j[w])/V,aa=(r[B]-r[w])/V,ha=(D[B]-D[w])/V;V=(p[B]-p[w])/V;var ma=j[B],ta=r[B],za=D[B],X=p[B],Y=o[B]-o[K];Y=Y!=0?Y:1;var da=(j[B]-j[K])/Y,fa=(r[B]-r[K])/Y,ra=(D[B]-D[K])/Y;Y=(p[B]-p[K])/Y;var na=j[K],O=r[K],la=D[K],ya=p[K],wa=o[K]-o[w];wa=wa!=0?wa:1;var Ga=(j[K]-j[w])/
wa,Z=(r[K]-r[w])/wa,oa=(D[K]-D[w])/wa;wa=(p[K]-p[w])/wa;var pa=o[B]*c;for(B=o[B];B>o[w];B--){if(B>=0&&B<f){var J=~~sa,P=M,ba=S,Q=A,$,T,ua,va;if(B>o[K]){$=~~ma;T=ta;ua=za;va=X}else{$=~~na;T=O;ua=la;va=ya}if(J>$){var W;W=J;J=$;$=W;W=P;P=T;T=W;W=ba;ba=ua;ua=W;W=Q;Q=va;va=W}T=J!=$?(T-P)/($-J):1;ua=J!=$?(ua-ba)/($-J):1;va=J!=$?(va-Q)/($-J):1;if(J<0){P-=J*T;ba-=J*ua;Q-=J*va;J=0}if($>=c)$=c-1;W=pa+J;if(R){J=J;P=P;ba=ba;for(Q=Q;J<=$;J++,P+=T,ba+=ua,Q+=va){if(P>i[W]){i[W]=P;var L=G[(Q&t)*x+(ba&t)],ia=((U&
16711680)>>16)*((L&16711680)>>8),qa=((U&65280)>>8)*((L&65280)>>8),ca=(U&255)*(L&255)>>8;h[W]=ia&16711680|qa&65280|ca&255;m[W]=q}W++}}else{J=J;P=P;ba=ba;for(Q=Q;J<$;J++,P+=T,ba+=ua,Q+=va){if(P>i[W]){ca=G[(Q&t)*x+(ba&t)];L=h[W];var ja=(ca>>24&255)*(C&255)>>8;ia=((U&16711680)>>16)*((ca&16711680)>>8);qa=((U&65280)>>8)*((ca&65280)>>8);ca=(U&255)*(ca&255)>>8;if(ja>250)i[W]=P;else{var ga=255-ja;ia=ia*ja+(L&16711680)*ga>>8;qa=qa*ja+(L&65280)*ga>>8;ca=ca*ja+(L&255)*ga>>8}h[W]=ia&16711680|qa&65280|ca&255;m[W]=
q}W++}}}sa-=ka;M-=aa;S-=ha;A-=V;if(B>o[K]){ma-=da;ta-=fa;za-=ra;X-=Y}else{na-=Ga;O-=Z;la-=oa;ya-=wa}pa-=c}}w=z;K=H}while(g[l]!=-1);l++}}}};
a.renderTextureSmooth=function(b){var c=this.frameWidth,f=this.frameHeight,g=b.indexBuffer,d=b.transformedVertexBuffer,e=b.transformedVertexNormalZBuffer,h=b.transformedFaceNormalZBuffer,i=this.colorBuffer,m=this.zBuffer,n=this.selectionBuffer,q=b.faceCount,j=b.internalId,y=d.length/3,k=b.material?b.material:this.defaultMaterial,R=k.getPalette(),C=b.texture,s=k.transparency==0&&!C.hasTransparency,E=~~((1-k.transparency)*255),G=b.texCoordBuffer,x=b.texCoordIndexBuffer,t=C.data,u=C.width,v=u-1,o=C.hasMipmap()?
C.mipmaps:null,r=o?C.mipentries:null;if(k.transparency!=1){if(!e||e.length<y){b.transformedVertexNormalZBuffer=new Array(y);e=b.transformedVertexNormalZBuffer}if(!h||h.length<q){b.transformedFaceNormalZBuffer=new Array(q);h=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.vertexNormalBuffer,e);JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.faceNormalBuffer,h);b=b.isDoubleSided;y=new Array(3);k=new Array(3);for(var D=new Array(3),p=new Array(3),I=new Array(3),l=new Array(3),
U=0,N=0;U<q;){var w=h[U++];if(b)w=w>0?w:-w;if(w<0){do;while(g[N++]!=-1)}else{var z,F,K,H,B,sa,M,S;w=g[N];K=w*3;sa=x[N]*2;N++;z=g[N];H=z*3;M=x[N]*2;N++;if(o){B=g[N]*3;S=x[N]*2;u=C.width;y[0]=d[K];k[0]=d[K+1];y[1]=d[H];k[1]=d[H+1];y[2]=d[B];k[2]=d[B+1];I[0]=G[sa]*u;l[0]=G[sa+1]*u;I[1]=G[M]*u;l[1]=G[M+1]*u;I[2]=G[S]*u;l[2]=G[S+1]*u;t=(y[1]-y[0])*(k[2]-k[0])-(k[1]-k[0])*(y[2]-y[0]);if(t<0)t=-t;t+=1;v=(I[1]-I[0])*(l[2]-l[0])-(l[1]-l[0])*(I[2]-I[0]);if(v<0)v=-v;t=v/t;v=0;if(t<r[1])v=0;else if(t>=r[r.length-
1]){v=r.length-1;u=1}else for(;t>=r[v+1];){v++;u/=2}t=o[v];v=u-1}do{F=g[N];B=F*3;S=x[N]*2;N++;y[0]=~~(d[K]+0.5);k[0]=~~(d[K+1]+0.5);D[0]=d[K+2];y[1]=~~(d[H]+0.5);k[1]=~~(d[H+1]+0.5);D[1]=d[H+2];y[2]=~~(d[B]+0.5);k[2]=~~(d[B+1]+0.5);D[2]=d[B+2];I[0]=G[sa]*u;l[0]=G[sa+1]*u;I[1]=G[M]*u;l[1]=G[M+1]*u;I[2]=G[S]*u;l[2]=G[S+1]*u;p[0]=e[w];p[1]=e[z];p[2]=e[F];if(b){if(p[0]<0)p[0]=-p[0];if(p[1]<0)p[1]=-p[1];if(p[2]<0)p[2]=-p[2]}z=k[0]<k[1]?0:1;z=k[z]<k[2]?z:2;var A=k[0]>k[1]?0:1;A=k[A]>k[2]?A:2;H=3-A-z;if(z!=
A){M=y[A];var V=D[A],ka=I[A],aa=l[A],ha=p[A]*255,ma=k[A]-k[z];ma=ma!=0?ma:1;var ta=(y[A]-y[z])/ma,za=(D[A]-D[z])/ma,X=(I[A]-I[z])/ma,Y=(l[A]-l[z])/ma;ma=(p[A]-p[z])*255/ma;var da=y[A],fa=D[A],ra=I[A],na=l[A],O=p[A]*255,la=k[A]-k[H];la=la!=0?la:1;var ya=(y[A]-y[H])/la,wa=(D[A]-D[H])/la,Ga=(I[A]-I[H])/la,Z=(l[A]-l[H])/la;la=(p[A]-p[H])*255/la;var oa=y[H],pa=D[H],J=I[H],P=l[H],ba=p[H]*255,Q=k[H]-k[z];Q=Q!=0?Q:1;var $=(y[H]-y[z])/Q,T=(D[H]-D[z])/Q,ua=(I[H]-I[z])/Q,va=(l[H]-l[z])/Q;Q=(p[H]-p[z])*255/Q;
var W=k[A]*c;for(A=k[A];A>k[z];A--){if(A>=0&&A<f){var L=~~M,ia=V,qa=ka,ca=aa,ja=ha,ga,Aa,Ba,Ca,Da;if(A>k[H]){ga=~~da;Aa=fa;Ba=ra;Ca=na;Da=O}else{ga=~~oa;Aa=pa;Ba=J;Ca=P;Da=ba}if(L>ga){var ea;ea=L;L=ga;ga=ea;ea=ia;ia=Aa;Aa=ea;ea=qa;qa=Ba;Ba=ea;ea=ca;ca=Ca;Ca=ea;ea=ja;ja=Da;Da=ea}Aa=L!=ga?(Aa-ia)/(ga-L):1;Ba=L!=ga?(Ba-qa)/(ga-L):1;Ca=L!=ga?(Ca-ca)/(ga-L):1;Da=L!=ga?(Da-ja)/(ga-L):0;if(L<0){ia-=L*Aa;qa-=L*Ba;ca-=L*Ca;ja-=L*Da;L=0}if(ga>=c)ga=c-1;ea=W+L;if(s){L=L;ia=ia;ja=ja;qa=qa;for(ca=ca;L<=ga;L++,
ia+=Aa,ja+=Da,qa+=Ba,ca+=Ca){if(ia>m[ea]){m[ea]=ia;var xa=R[ja>0?~~ja:0],Fa=t[(ca&v)*u+(qa&v)],Ha=((xa&16711680)>>16)*((Fa&16711680)>>8),Ia=((xa&65280)>>8)*((Fa&65280)>>8);xa=(xa&255)*(Fa&255)>>8;i[ea]=Ha&16711680|Ia&65280|xa&255;n[ea]=j}ea++}}else{L=L;ia=ia;ja=ja;qa=qa;for(ca=ca;L<ga;L++,ia+=Aa,ja+=Da,qa+=Ba,ca+=Ca){if(ia>m[ea]){xa=R[ja>0?~~ja:0];var Ea=t[(ca&v)*u+(qa&v)];Fa=i[ea];var Ja=(Ea>>24&255)*(E&255)>>8;Ha=((xa&16711680)>>16)*((Ea&16711680)>>8);Ia=((xa&65280)>>8)*((Ea&65280)>>8);xa=(xa&255)*
(Ea&255)>>8;if(Ja>250)m[ea]=ia;else{Ea=255-Ja;Ha=Ha*Ja+(Fa&16711680)*Ea>>8;Ia=Ia*Ja+(Fa&65280)*Ea>>8;xa=xa*Ja+(Fa&255)*Ea>>8}i[ea]=Ha&16711680|Ia&65280|xa&255;n[ea]=j}ea++}}}M-=ta;V-=za;ka-=X;aa-=Y;ha-=ma;if(A>k[H]){da-=ya;fa-=wa;ra-=Ga;na-=Z;O-=la}else{oa-=$;pa-=T;J-=ua;P-=va;ba-=Q}W-=c}}z=F;H=B;M=S}while(g[N]!=-1);N++}}}};a.params=null;a.canvas=null;a.ctx=null;a.canvasData=null;a.bkgColorBuffer=null;a.colorBuffer=null;a.zBuffer=null;a.selectionBuffer=null;a.frameWidth=0;a.frameHeight=0;
a.scene=null;a.defaultMaterial=null;a.isLoaded=false;a.isFailed=false;a.errorMsg="";a.initRotX=0;a.initRotY=0;a.initRotZ=0;a.zoomFactor=1;a.rotMatrix=null;a.transformMatrix=null;a.sceneUrl="";a.modelColor=13280792;a.bkgColor1=16777215;a.bkgColor2=16777088;a.renderMode="flat";a.definition="standard";a.isMipMappingOn=false;a.buttonStates=null;a.keyStates=null;a.mouseX=0;a.mouseY=0;a.onmousedown=null;a.onmouseup=null;a.onmousemove=null;a.isDefaultInputHandlerEnabled=false;
...
255;this.palette[h++]=i<<16|m<<8|n}}else for(h=0;h<256;){i=b+h*g/256;m=c+h*d/256;n=f+h*e/256;if(i>255)i=255;if(m>255)m=255;if(n>255)n=255;this.palette[h++]=i<<16|m<<8|n}};a.name="";a.ambientColor=0;a.diffuseColor=8355711;a.transparency=0;a.simulateSpecular=false;a.palette=null;JSC3D.Texture=function(){this.name="";this.height=this.width=0;this.mipentries=this.mipmaps=this.data=null;this.hasTransparency=false;this.onready=null};a=JSC3D.Texture.prototype;
a.createFromUrl=function(b,c){var f=this,g=new Image;g.onload=function(){f.data=null;f.mipmaps=null;f.mipentries=null;f.width=0;f.height=0;f.hasTransparency=false;f.createFromImage(this,c)};g.onerror=function(){f.data=null;f.mipmaps=null;f.mipentries=null;f.width=0;f.height=0;f.hasTransparency=false};g.src=b};
a.createFromImage=function(b,c){if(!(b.width<=0||b.height<=0)){var f=false,g=JSC3D.Texture.cv;if(!g)try{g=document.createElement("canvas");JSC3D.Texture.cv=g;f=true}catch(d){return}var e=b.width>b.height?b.width:b.height;e=e<=32?32:e<=64?64:e<=128?128:e<=256?256:512;if(g.width!=e||g.height!=e){g.width=g.height=e;f=true}var h;try{var i=g.getContext("2d");f||i.clearRect(0,0,e,e);i.drawImage(b,0,0,e,e);h=i.getImageData(0,0,e,e).data}catch(m){return}b=h.length/4;this.data=new Array(b);for(i=g=0;g<b;g++,
i+=4){f=h[i+3];this.data[g]=f<<24|h[i]<<16|h[i+1]<<8|h[i+2];if(f<255)this.hasTransparency=true}this.height=this.width=e;this.mipmaps=null;c&&this.generateMipmaps();this.onready!=null&&typeof this.onready=="function"&&this.onready()}};a.hasData=function(){return this.data!=null};
a.generateMipmaps=function(){if(!(this.width<=1||this.data==null||this.mipmaps!=null)){this.mipmaps=[this.data];this.mipentries=[1];for(var b=1+~~(0.1+Math.log(this.width)*Math.LOG2E),c=this.width>>1,f=1;f<b;f++){for(var g=new Array(c*c),d=this.mipmaps[f-1],e=c<<1,h=0,i=0,m=0;m<c;m++){for(var n=0;n<c;n++){var q=d[h],j=d[h+1],y=d[h+e],k=d[h+e+1];g[i]=(((q&4278190080)>>>2)+((j&4278190080)>>>2)+((y&4278190080)>>>2)+((k&4278190080)>>>2)&4278190080)+((q&16711680)+(j&16711680)+(y&16711680)+(k&16711680)>>
2&16711680)+((q&65280)+(j&65280)+(y&65280)+(k&65280)>>2&65280)+((q&255)+(j&255)+(y&255)+(k&255)>>2&255);h+=2;i++}h+=e}this.mipmaps.push(g);this.mipentries.push(Math.pow(4,f));c>>=1}}};a.hasMipmap=function(){return this.mipmaps!=null};a.name="";a.data=null;a.mipmaps=null;a.mipentries=null;a.width=0;a.height=0;a.hasTransparency=false;a.onready=null;JSC3D.Texture.cv=null;JSC3D.AABB=function(){this.minZ=this.maxZ=this.minY=this.maxY=this.minX=this.maxX=0};
...
a.parseObj=function(b,c){var f={},g=[],d=0,e=null,h="",i="",m=[],n=[];e="obj-"+d++;var q=new JSC3D.Mesh;q.name=e;q.indexBuffer=[];e=f.nomtl=q;var j=c.split("\n");for(c=0;c<j.length;c++){i=j[c].split(" ");if(i.length>0)switch(i[0]){case "v":for(var y=1;y<i.length;y++)m.push(parseFloat(i[y]));break;case "vn":break;case "vt":n.push(parseFloat(i[1]));n.push(1-parseFloat(i[2]));break;case "f":for(y=1;y<i.length;y++){var k=i[y].split("/");e.indexBuffer.push(parseInt(k[0])-1);if(k.length>1&&k[1]!=""){if(!e.texCoordIndexBuffer)e.texCoordIndexBuffer=
[];e.texCoordIndexBuffer.push(parseInt(k[1])-1)}}e.indexBuffer.push(-1);e.texCoordIndexBuffer&&e.texCoordIndexBuffer.push(-1);break;case "mtllib":if(i.length>1){h=i[1];g.push(h)}else h="";break;case "usemtl":if(i.length>1&&i[1]!=""&&h!=""){i=i[1];y=h+"-"+i;e=f[y];if(!e){e=new JSC3D.Mesh;e.name="obj-"+d++;e.indexBuffer=[];e.mtllib=h;e.mtl=i;f[y]=e}e=e}else e=q;break;case "#":default:break}}d=m.length>=3?new Array(m.length/3):null;h=n.length>=2?new Array(n.length/2):null;for(id in f){e=f[id];if(m.length>=
